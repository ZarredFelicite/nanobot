...
class TestArchiveAllMode:
    """Test archive_all mode (used by /compact command)."""
...
class TestConsolidationDeduplicationGuard:
    ...
    @pytest.mark.asyncio
    async def test_clear_command_clears_session(
        self, tmp_path: Path
    ) -> None:
        """/clear clears session without triggering consolidation."""
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        from nanobot.providers.base import LLMResponse
        ...
        clear_msg = InboundMessage(channel="cli", sender_id="user", chat_id="test", content="/clear")
        response = await loop._process_message(clear_msg)
